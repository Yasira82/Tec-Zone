...
// A proper app Settings page for Zone (modeled on tec-assets' settings): sectioned,
  ...
  return (
    <div>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: TEC_COLORS.text }}>
              {/* While `/api/auth/me` is still answering, say nothing — not
                  "Not signed in". That verdict was being painted on every
                  first render and replaced a moment later, so the same app
                  showed the name on one visit and "Not signed in" on the next,
                  depending only on how fast the phone was and when you looked. */}
      ...
      <Section title={s.about} icon="ℹ️">
        ...
        <Row label={s.domain}><span style={{ color: TEC_COLORS.subtext, fontSize: 14 }}>zone.pi</span></Row>
