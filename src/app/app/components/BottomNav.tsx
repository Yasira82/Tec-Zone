...
import { Icon, type ZoneIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type ZoneTab = 'home' | 'registry' | 'verify' | 'settings';

export function BottomNav({ active, onSelect }: { active: ZoneTab; onSelect: (t: ZoneTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: ZoneTab; icon: ZoneIconName; label: string }[] = [
    { key: 'home',     icon: 'shield',   label: t.zone.nav.home     },
    { key: 'registry', icon: 'grid',     label: t.zone.nav.registry },
    { key: 'verify',   icon: 'verify',   label: t.zone.nav.verify   },
    { key: 'settings', icon: 'settings', label: t.zone.nav.settings },
  ];
  return (
    <nav style={{
